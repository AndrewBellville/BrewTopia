package com.dev.town.small.brewtopia.Utilites.RangeBar;
...
public class RangeSeekBar<T extends Number> extends ImageView {
    ...
    private static final int INITIAL_PADDING_IN_DP = 4;
    private static final int DEFAULT_TEXT_SIZE_IN_DP = 14;
    private static final int DEFAULT_TEXT_DISTANCE_TO_BUTTON_IN_DP = 3;
    private static final int DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP = 3;
    ...
    private Bitmap thumbCurrentImage;
    ...
    protected double normalizedCurrentValue = 1d;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        int thumbCurrent = R.drawable.seek_thumb_current;
        ...
        if (attrs == null) {
            ...
            try {
                ...
                activeColor = a.getColor(R.styleable.RangeSeekBar_activeColor, getResources().getColor(R.color.AccentColor));
                ...
                Drawable disabledDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbCurrent);
                if (disabledDrawable != null) {
                    thumbCurrentImage = BitmapUtil.drawableToBitmap(disabledDrawable);
                }
        ...
        if (thumbCurrentImage == null) {
            thumbCurrentImage = BitmapFactory.decodeResource(getResources(), thumbCurrent);
        }
    ...
    /**
     * Sets the currently value. The widget will be invalidated and redrawn.
     *
     * @param value The Number value to set current value to. Will be clamped to given absolute minimum/maximum range.
     */
    ...
    public T getSelectedCurrentValue() {
        return roundOffValueToStep(normalizedToValue(normalizedCurrentValue));
    }
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        drawThumb(normalizedToScreen(normalizedCurrentValue), true, canvas,
                selectedValuesAreDefault);
        ...
        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            String curText = valueToString(getSelectedCurrentValue());
            float minTextWidth = paint.measureText(minText);
            float maxTextWidth = paint.measureText(maxText);
            float curTextWidth = paint.measureText(curText);
            ...
            float curPosition = Math.max(0f, normalizedToScreen(normalizedCurrentValue) - curTextWidth * 0.5f);
    ...
    private void drawThumb(float screenCoord, boolean pressed, Canvas canvas, boolean areSelectedValuesDefault) {
        ...
        if (!activateOnDefaultValues && areSelectedValuesDefault) {
            buttonToDraw = thumbCurrentImage;
        } else {
            buttonToDraw = pressed ? thumbCurrentImage : thumbImage;
        }
    ...
    /**
     * Sets normalized current value to value so that 0 <= normalized min value <= value <= 1. The View will get invalidated when calling this method.
     *
     * @param value The new normalized current value to set.
     */
    private void setNormalizedCurrentValue(double value) {
        normalizedCurrentValue = value;// Math.max(0d, Math.min(1d, Math.max(value, normalizedMinValue)));
        invalidate();
    }
